...
function SalesCard() {
  ...
  useEffect(()=>{
    const minSearchDate = minDate.toISOString().slice(0, 10)
    const mixSearchDate = maxDate.toISOString().slice(0, 10)

    axios.get(`${BASE_URL}/sales?minDate=${minSearchDate}&maxDate=${mixSearchDate}`)
      .then(response => {
